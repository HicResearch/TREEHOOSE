...
class EgressBackendStack(cdk.Stack):
    ...
    def __init__(self, scope: cdk.Construct, construct_id: str, env_id: str, **kwargs) -> None:
        ...
        swb_egress_notification_bucket_kms_key = kms.Key.from_key_arn(
            ...
            self.node.try_get_context(env_id).get("swb_egress_notification_bucket_kms_arn")
        )
        ...
        sns_configuration_set_destination = EmailConfigurationSetEventDestinationCustomResource(
            self,
            "EmailConfigurationSetEventDestinationCustomResource",
            account_id=self.account,
            region=self.region,
        ...
        ec2.FlowLog(
            ...
            destination=ec2.FlowLogDestination.to_cloud_watch_logs(vpc_log_group, vpc_flow_log_role)
        )
        ...
        egress_requests_table = ddb.Table(
            ...
            partition_key=ddb.Attribute(name='egress_request_id', type=ddb.AttributeType.STRING),
        ...
        egress_webapp_redeploy_lambda_policy = iam.ManagedPolicy(
            ...
            statements=[
                ...
                iam.PolicyStatement(
                    ...
                    resources=[f"arn:aws:amplify:{self.region}:{self.account}:apps/*/branches/*"]
                )
            ]
        )

        egress_webapp_redeploy_lambda_role.add_managed_policy(egress_webapp_redeploy_lambda_policy)
        ...
        egress_webapp_redeploy_function = lmb.Function(
            ...
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/egress_webapp_redeploy')),
        ...
        ig_update_request_in_db_task = sfn_tasks.DynamoUpdateItem(
            ...
            expression_attribute_values={
                ":reason": sfn_tasks.DynamoAttributeValue.from_string(
                    sfn.JsonPath.string_at("$.information_governance.result.ig_reviewer_1_reason")
                ),
                ":decision": sfn_tasks.DynamoAttributeValue.from_string(
                    sfn.JsonPath.string_at("$.information_governance.result.ig_reviewer_1_decision")
                ),
                ":email": sfn_tasks.DynamoAttributeValue.from_string(
                    sfn.JsonPath.string_at("$.information_governance.result.ig_reviewer_1_email")
                ),
                ":review_date": sfn_tasks.DynamoAttributeValue.from_string(
                    sfn.JsonPath.string_at("$.information_governance.result.ig_reviewer_1_dt")
                ),
                ":req_status": sfn_tasks.DynamoAttributeValue.from_string(
                    sfn.JsonPath.string_at("$.information_governance.result.egress_status")
                )
            },
            result_path=sfn.JsonPath.DISCARD
        )
        ...
        rit_update_request_in_db_task = sfn_tasks.DynamoUpdateItem(
            ...
            expression_attribute_values={
                ...
                ":decision": sfn_tasks.DynamoAttributeValue.from_string(
                    sfn.JsonPath.string_at("$.research_it.result.rit_reviewer_2_decision")
                ),
                ":email": sfn_tasks.DynamoAttributeValue.from_string(
        ...
        copy_staging_lambda_policy = iam.ManagedPolicy(
            ...
            statements=[
                ...
                iam.PolicyStatement(
                    ...
                    resources=[
                        swb_egress_notification_bucket_kms_key.key_arn
                    ]
                )
            ]
        )
        ...
        copy_egress_candidates_to_staging_function = lmb.Function(
            ...
            runtime=lmb.Runtime.PYTHON_3_8,
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/copy_egress_candidates_to_staging')),
            layers=[powertools_layer],
        ...
        notify_ig_reviewer_task = sfn_tasks.SnsPublish(
            ...
            topic=ig_role_topic,
            subject=sfn.JsonPath.string_at("States.Format('{} Egress Request', $.project_id)"),
            message=sfn.TaskInput.from_object({
                "Egress Request ID": sfn.JsonPath.string_at("$.egress_request_id"),
                "Researcher Email": sfn.JsonPath.string_at("$.created_by_email"),
                "Egress Object File Types": sfn.JsonPath.string_at("$.copy_to_staging_result.file_extensions")
            }),
            result_path=sfn.JsonPath.DISCARD
        )
        ...
        notify_rit_reviewer_task = sfn_tasks.SnsPublish(
            ...
            message=sfn.TaskInput.from_object({
                "Egress Request ID": sfn.JsonPath.string_at("$.egress_request_id"),
                "Researcher Email": sfn.JsonPath.string_at("$.created_by_email"),
                "Information Governance": sfn.JsonPath.string_at("$.information_governance.result.ig_reviewer_1_email"),
                "Egress Object File Types": sfn.JsonPath.string_at("$.copy_to_staging_result.file_extensions")
            }),
            result_path=sfn.JsonPath.DISCARD
        )
        ...
        update_egress_request_with_task_token_function = lmb.Function(
            ...
            runtime=lmb.Runtime.PYTHON_3_8,
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/update_egress_request')),
            layers=[powertools_layer],
            tracing=lmb.Tracing.ACTIVE,
        ...
        egress_requests_table.grant_read_write_data(update_egress_request_with_task_token_function)
        ...
        ig_decision_task = sfn_tasks.LambdaInvoke(
            ...
            payload=sfn.TaskInput.from_object({
                ...
                "current_reviewer_group": sfn.JsonPath.string_at("$.reviewer_list[0]")
            }),
            result_selector={
                "result": sfn.JsonPath.string_at("$.request")
            },
        ...
        rit_decision_task = sfn_tasks.LambdaInvoke(
            ...
            payload=sfn.TaskInput.from_object({
                "token": sfn.JsonPath.task_token,
                "input": sfn.JsonPath.string_at("$.egress_request_id"),
                "current_reviewer_group": sfn.JsonPath.string_at("$.reviewer_list[1]")
            }),
            result_selector={
                "result": sfn.JsonPath.string_at("$.request")
            },
        ...
        handle_egress_rejection_function = lmb.Function(
            ...
            runtime=lmb.Runtime.PYTHON_3_8,
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/handle_egress_rejection')),
            layers=[powertools_layer],
            tracing=lmb.Tracing.ACTIVE,
            timeout=Duration.seconds(10),
            environment={
        ...
        copy_egress_candidates_to_datalake_function = lmb.Function(
            ...
            runtime=lmb.Runtime.PYTHON_3_8,
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/copy_egress_candidates_to_datalake')),
            layers=[powertools_layer],
            tracing=lmb.Tracing.ACTIVE,
            ...
            vpc=vpc,
            filesystem=lmb.FileSystem.from_efs_access_point(file_access_point, efs_mount_path)
        )
        ...
        datalake_bucket_kms_key.grant_encrypt_decrypt(copy_egress_candidates_to_datalake_function)
        ...
        pre_token_generation_fn = lmb.Function(
            ...
            runtime=lmb.Runtime.PYTHON_3_8,
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/pre_token_generation')),
            layers=[powertools_layer],
            tracing=lmb.Tracing.ACTIVE,
            timeout=Duration.seconds(10),
            environment={
                "REVIEWER_LIST": json.dumps(self.node.try_get_context(env_id).get("egress_reviewer_roles")),
                "REGION": self.region
            }
        )
        ...
        egress_user_pool = cognito.UserPool(
            ...
            lambda_triggers={
                "pre_token_generation": pre_token_generation_fn
            },
            mfa=cognito.Mfa.OFF
        )
        ...
        cognito_userpool_domain = self.node.try_get_context(env_id).get("cognito_userpool_domain")
        ...
        send_final_decision_email = sfn_tasks.CallAwsService(
            ...
            parameters={
                ...
                "Destination": {
                    "ToAddresses": sfn.JsonPath.string_at("States.Array($.created_by_email)"),
                    "CcAddresses": sfn.JsonPath.string_at("States.Array($.information_governance.result.ig_reviewer_1_email)")
                },
                "Content": {
                    "Simple": {
                        "Subject": {
                            "Data": sfn.JsonPath.string_at("States.Format('{} Egress Request Review Completed', $.project_id)")
                        },
                        "Body": {
                            "Text": {
            ...
            iam_resources=[f"arn:aws:ses:{self.region}:{self.account}:identity/{tre_admin_email_address}"],
        ...
        data_egress_step_function = sfn.StateMachine(
            ...
            definition=add_request_to_db_task.next(
                ...
                notify_ig_reviewer_task
            ).next(
                ig_decision_task
            ).next(
                ig_update_request_in_db_task
            ).next(
                check_ig_approval.when(
                    sfn.Condition.string_matches("$.information_governance.result.egress_status", "*APPROVED"),
                    notify_rit_reviewer_task.next(rit_decision_task).next(
                        rit_update_request_in_db_task
                    ).next(
                        check_rit_approval.when(
                            sfn.Condition.string_equals("$.research_it.result.rit_reviewer_2_decision", "APPROVED"),
                            copy_to_datalake_task.next(
                                update_request_in_egress_store_db_rit_task
                            )
                        ).when(
                            sfn.Condition.string_equals("$.research_it.result.rit_reviewer_2_decision", "REJECTED"),
                    ...
                    sfn.Condition.string_matches("$.information_governance.result.egress_status", "*REJECTED"),
        ...
        start_egress_workflow_function = lmb.Function(
            ...
            runtime=lmb.Runtime.PYTHON_3_8,
            code=lmb.Code.from_asset(path.join(this_dir, 'lambda/start_egress_workflow')),
            layers=[powertools_layer],
            tracing=lmb.Tracing.ACTIVE,
            timeout=Duration.seconds(10),
            environment={
        ...
        swb_egress_topic.add_subscription(subscriptions.LambdaSubscription(start_egress_workflow_function))
        ...
        egress_api_handler = lmb.Function(
            ...
            environment={
                ...
                "REVIEWER_LIST": json.dumps(self.node.try_get_context(env_id).get("egress_reviewer_roles")),
                "MAX_DOWNLOADS_ALLOWED": self.node.try_get_context(env_id).get("max_downloads_allowed")
            }
        )
        ...
        lambda_ds = appsync_api.add_lambda_data_source("lambdaDataSource", egress_api_handler)
        ...
        NagSuppressions.add_resource_suppressions(
            ...
            [
                {
                    "id": "AwsSolutions-IAM5",
                    "reason": "Wilcard permissions are expected"
                }
            ],
            True
        )
