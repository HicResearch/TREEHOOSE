...
def update_request(arguments: str, context: Any):
    ...
    logger.info("Update Request API invoked with Egress Request ID: " + inbound_egress_request_id)
    ...
    reviewer_valid = is_reviewer_valid(request_id=inbound_egress_request_id, reviewer_usergroup=usergroup, egress_request=egress_details)

    # Check if request is valid
    request_valid = is_request_valid(request_id=inbound_egress_request_id, task_token=inbound_task_token, egress_request=egress_details)

    # Determine egress request status and SWB status
    statuses = determine_status(egress_arguments=arguments, reviewer_usergroup=usergroup)
    ...
    if request_valid and reviewer_valid:
        ...
        step_fn_client.send_task_success(
            taskToken=arguments['request']['task_token'],
            output=json.dumps(arguments)
        )

        logger.info("Request updated successfully and restarting stepfunction workflow...")
...
def determine_status(egress_arguments: Any, reviewer_usergroup: str):
    ...
    if reviewer_usergroup == reviewer_list_groups[0]:
        inbound_reviewer_1_decision = egress_arguments['request']['ig_reviewer_1_decision']
        ...
        inbound_reviewer_1_decision = egress_arguments['request']['ig_reviewer_1_decision']
        inbound_reviewer_2_decision = egress_arguments['request']['rit_reviewer_2_decision']

        if inbound_reviewer_1_decision == 'APPROVED' and inbound_reviewer_2_decision == 'REJECTED':
            egress_status = 'REJECTED'
            swb_status = 'PENDING'
        elif inbound_reviewer_1_decision == 'APPROVED' and inbound_reviewer_2_decision == 'APPROVED':
        ...
        raise Exception('Unable to determine the status of the request. Please refresh and retry')

    return {
        "egress_status": egress_status,
        "swb_status": swb_status
    }
...
def retrieve_request_details(request_id: str):
    ...
    if response['Items'] and len(response['Items']) == 1:
        ...
        raise Exception(f'No matching egress request found for egress request: {request_id}')
...
def is_reviewer_valid(request_id: str, reviewer_usergroup: str, egress_request: Any):
    ...
    if current_reviewer_group == reviewer_usergroup:
        logger.info("Reviewer is in current reviewer group: %s and is valid", current_reviewer_group)
        return True
    else:
        logger.error(
...
def is_request_valid(request_id: str, task_token: str, egress_request: Any):
    if egress_request['Items'][0]['task_token'] == task_token:
        ...
        logger.error("Egress request: %s found but task token %s is not valid", request_id, task_token)
        raise Exception('Another user may have updated this request. Please refresh to get the latest data')
