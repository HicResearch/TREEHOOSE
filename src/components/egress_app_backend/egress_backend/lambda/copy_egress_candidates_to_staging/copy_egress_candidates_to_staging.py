...
def handler(event, context):
    ...
    logger.info('Starting copy to staging bucket with egress request ID: ' + egress_request_id)
    ...
    return {
        "file_extensions": list(extension_set),  # convert set to list which is easily serializable into JSON
        "staged": True
    }
...
def copy_files_to_egress_staging(version_list: object, source_bucket: str, workspace: str, egress_request_id: str):
    object_list = []
    get_objects_list(source_bucket, workspace, object_list)
    if object_list:
        copy_objects(version_list, object_list, target_bucket, source_bucket, workspace, egress_request_id)
    else:
...
####################################################################
def copy_objects(version_list: object, object_list: str, target_bucket: str, source_bucket: str, workspace_id: str, egress_request_id: str):
    """
    ...
    for _counter, obj in enumerate(object_list, start=1):
        ...
        versioned_object = next(object_k for object_k in version_list['objects'] if object_k["Key"] == obj)
